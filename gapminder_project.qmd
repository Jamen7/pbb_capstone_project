...
# Merge child_mortality_long with fertility_long on both country and year
...
To include the `employ_long` to the `child_fertility_sub`, both datasets need a uniform country code to ensure consistency.
...
# Dropdown widget
countries = child_fertility['country'] #["China", "Egypt", "Australia", "Argentina", "Germany"]
...
def update_plot(selected_countries):
    filtered_df = child_fertility.query("country in @selected_countries")
    fig = px.scatter(
        filtered_df,
        x="year",
        y="woman_fertility",
...
widgets.interactive(update_plot, selected_countries=country_selector)
...
home_layout = html.Div([
    ...
    html.P("Navigate to different pages using the links below."),
    dcc.Link("Go to Fertility Plot", href="/fertility"),
    html.Br(),
    dcc.Link("Go to Mortality Plot", href="/mortality"),
])

# Define the layout for the fertility page
fertility_layout = html.Div([
    html.H1("Fertility Plot"),
    dcc.Link("Go back to Home", href="/"),
    dcc.Graph(id="fertility-plot"),
])

# Define the layout for the mortality page
mortality_layout = html.Div([
    html.H1("Mortality Plot"),
    dcc.Link("Go back to Home", href="/"),
...
def display_page(pathname):
    if pathname == "/fertility":
        return fertility_layout
    elif pathname == "/mortality":
        return mortality_layout
    else:
        return home_layout

# Add interactivity for plots
@app.callback(
    Output("fertility-plot", "figure"),
    Input("url", "pathname")
)
def update_fertility_plot(pathname):
    if pathname == "/fertility":
        fig = px.scatter(
            child_fertility.query("country in ['China', 'Egypt', 'Australia', 'Argentina', 'Germany']"),
            x="year",
            y="woman_fertility",
            color="country",
            size="mortality",
            title="Fertility Over Time",
        )
        return fig
    return {}
...
def update_mortality_plot(pathname):
